...
export const TutorialProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return (
        <TutorialContext.Provider value={{ activeTutorial, currentStepIdx, startTutorial, completeTutorial, nextStep }}>
            {children}
            {activeTutorial && targetRect && steps[currentStepIdx] && (
                <div className="fixed inset-0 z-[9999] pointer-events-none">
                    ...
                    <div 
                        ...
                        style={{
                            left: steps[currentStepIdx].position === 'right' ? targetRect.right + 24 : 
                                  steps[currentStepIdx].position === 'left' ? targetRect.left - 312 : 
                                  targetRect.left + (targetRect.width / 2) - 144,
                            top: steps[currentStepIdx].position === 'bottom' ? targetRect.bottom + 24 :
                                 steps[currentStepIdx].position === 'top' ? targetRect.top - 220 : 
                    ...
                    >
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-widest">Neural Assist</span>
                        </div>
                        <h4 className="text-sm font-display font-bold text-nexus-text mb-2">{steps[currentStepIdx].title}</h4>
                        <p className="text-xs text-nexus-muted leading-relaxed font-serif italic mb-6">"{steps[currentStepIdx].content}"</p>
                        
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex gap-1">
                                {steps.map((_, i) => (
                                    <div key={i} className={`h-1 rounded-full transition-all ${i === currentStepIdx ? 'w-4 bg-nexus-accent' : 'w-1 bg-nexus-800'}`} />
                                ))}
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => completeTutorial(activeTutorial!)} className="p-2 text-nexus-muted hover:text-red-400 transition-colors">
                                    <X size={14} />
                                </button>
                                {!steps[currentStepIdx].isInteraction && (
                                    ...
                                    >
                                        {currentStepIdx === steps.length - 1 ? 'Finish' : 'Next'} <ChevronRight size={12} />
                                    </button>
                                )}
                            </div>
                        </div>
