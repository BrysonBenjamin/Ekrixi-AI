...
export const DrilldownCanvas: React.FC<DrilldownCanvasProps> = ({ 
    ...
    const integrityMap = useMemo(() => GraphIntegrityService.getRegistryIntegrityMap(fullRegistry), [fullRegistry]);
    ...
    useEffect(() => {
        const zoom = d3.zoom<SVGSVGElement, unknown>().scaleExtent([0.005, 5]).on("zoom", (event) => setZoomTransform(event.transform));
        zoomRef.current = zoom;
        if (canvasRef.current) d3.select(canvasRef.current).call(zoom as any).on("dblclick.zoom", null);
    }, []);
        ...
        <div ref={containerRef} className="w-full h-full relative cursor-grab active:cursor-grabbing overflow-hidden bg-nexus-950">
            ...
            {linkingSourceId && (
                <div className="absolute top-8 left-1/2 -translate-x-1/2 z-[100] animate-in slide-in-from-top-4 duration-500">
                    <div className="px-6 py-3 bg-nexus-accent text-white rounded-full font-display font-black text-[10px] uppercase tracking-[0.4em] shadow-2xl flex items-center gap-4">
                        <Link2 size={16} /> Establish Connection Origin: {(registry[linkingSourceId] as any).title}
                        <button onClick={() => setLinkingSourceId(null)} className="p-1 hover:bg-white/20 rounded-full transition-colors"><X size={14} /></button>
                    </div>
                </div>
            )}
            <svg ref={canvasRef} className="w-full h-full block" onClick={() => { setContextMenu(null); setLockedId(null); setLinkingSourceId(null); }}>
                <g transform={zoomTransform.toString()}>
                    <defs>
                        <pattern id="nexus-grid-major" width="200" height="200" patternUnits="userSpaceOnUse"><path d="M 200 0 L 0 0 0 200" fill="none" stroke="var(--accent-color)" strokeWidth="0.5" opacity="0.08"/></pattern>
                        <pattern id="nexus-grid-minor" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="var(--ley-color)" strokeWidth="0.25"/></pattern>
                        <filter id="sigil-glow" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="15" result="blur" /><feComposite in="SourceGraphic" in2="blur" operator="over" /></filter>
                    </defs>
                    <rect x="-40000" y="-40000" width="80000" height="80000" fill="url(#nexus-grid-minor)" /><rect x="-40000" y="-40000" width="80000" height="80000" fill="url(#nexus-grid-major)" />
                    ...
                    <g className="links-layer">
                        {links.map((link: SimulationLink) => {
                            ...
                            if (sourceNode.x === undefined || sourceNode.y === undefined || targetNode.x === undefined || targetNode.y === undefined || isNaN(sourceNode.x)) return null;
                            
                            const isAnomalyFocus = integrityFocus?.linkId === link.id;
                            const isPartOfAnomalyPath = integrityFocus?.path && integrityFocus.path.includes(sourceNode.id) && integrityFocus.path.includes(targetNode.id);
                            ...
                            if (isTargetFocus && sourcePath === 'ancestor') { 
                                visualSource = targetNode; visualTarget = sourceNode; displayVerb = link.verbInverse || link.verb; 
                            } else if (isSourceFocus && targetPath === 'ancestor') { 
                                visualSource = sourceNode; visualTarget = targetNode; displayVerb = link.verbInverse || link.verb; 
                            } else if (isTargetFocus && sourcePath === 'descendant') { 
                                visualSource = targetNode; visualTarget = sourceNode; displayVerb = link.verb; 
                            }
                            ...
                            const baseFlowColor = isUpstream ? "var(--arcane-color)" : (isDownstream ? "var(--essence-color)" : "var(--accent-color)");
                            const flowColor = isAnomalyFocus ? "#ef4444" : isPartOfAnomalyPath ? "#06b6d4" : (isRedundant ? "#ef4444" : isImplied ? "#f59e0b" : baseFlowColor);
                            
                            const isRelevant = isAnomalyFocus || isPartOfAnomalyPath || hoveredLinkId === link.id || activeFilterId === sourceNode.id || activeFilterId === targetNode.id || focusId === sourceNode.id || focusId === targetNode.id;
                            const globalOpacity = isAnomalyFocus ? 1 : isPartOfAnomalyPath ? 0.9 : isRelevant ? (isRedundant ? 0.6 : 0.8) : 0.2;
                            
                            const dx = visualTarget.x! - visualSource.x!; const dy = visualTarget.y! - visualSource.y!; 
                            const dr = Math.sqrt(dx * dx + dy * dy) * 1.8;
                            const midX = (visualSource.x! + visualTarget.x!) / 2; const midY = (visualSource.y! + visualTarget.y!) / 2;

                            return (
                                <g key={link.id} className={`link-group transition-all duration-700 ${isAnomalyFocus ? 'animate-pulse' : ''}`} style={{ opacity: globalOpacity }} onMouseEnter={() => setHoveredLinkId(link.id)} onMouseLeave={() => setHoveredLinkId(null)}>
                                    <path d={`M${visualSource.x},${visualSource.y}A${dr},${dr} 0 0,1 ${visualTarget.x},${visualTarget.y}`} fill="none" stroke="transparent" strokeWidth="100" className="pointer-events-auto cursor-pointer" onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); if(!link.isStructuralLine) setContextMenu({ id: link.id, x: e.clientX, y: e.clientY }); }} />
                                    ...
                                    <path 
                                        ...
                                        fill="none" 
                                        stroke={flowColor} 
                                        strokeWidth={link.isStructuralLine ? 10 : isAnomalyFocus ? 14 : isPartOfAnomalyPath ? 10 : 4} 
                                    ...
                                    {(isRelevant || isAnomalyFocus || isPartOfAnomalyPath) && !link.isStructuralLine && (
                                        <foreignObject x={midX - 100} y={midY - 40} width="200" height="80" className="overflow-visible pointer-events-auto animate-in fade-in zoom-in-95 duration-500">
                                            <div className="w-full h-full flex items-center justify-center">
                                                <div onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); setContextMenu({ id: link.id, x: e.clientX, y: e.clientY }); }} className={`px-5 py-2.5 rounded-full border shadow-2xl transition-all duration-500 flex items-center gap-3 backdrop-blur-md cursor-pointer bg-nexus-900 border-nexus-800 ${isAnomalyFocus ? 'ring-4 ring-red-500/20' : ''}`} style={{ color: flowColor, textDecoration: isRedundant && !isAnomalyFocus ? 'line-through' : 'none' }}>
                                                    <span className="text-[10px] font-display font-black uppercase tracking-[0.25em]">{isAnomalyFocus ? 'ANOMALY: ' : isPartOfAnomalyPath ? 'CAUSAL PATH: ' : isRedundant ? '[REDUNDANT] ' : isImplied ? '[IMPLIED] ' : ''}{displayVerb}</span>
                                                </div>
                                            </div>
                                        </foreignObject>
                                    )}
                    ...
                    <g className="nodes-layer">
                        {nodes.map(node => {
                            const isPartOfAnomaly = integrityFocus?.path?.includes(node.id) || (integrityFocus && (fullRegistry[integrityFocus.linkId] as any).source_id === node.id) || (integrityFocus && (fullRegistry[integrityFocus.linkId] as any).target_id === node.id);
            ...
            {contextMenu && (
                <DrilldownContextMenu 
                    object={fullRegistry[contextMenu.id]} registry={fullRegistry} x={contextMenu.x} y={contextMenu.y} onClose={() => setContextMenu(null)} onInspect={onInspect} onDrilldown={onDrilldown} onDelete={onDelete} 
                    onReify={onReifyLink} onReifyNode={onReifyNode} onReifyNodeToLink={onReifyNodeToLink} onStartLink={(id) => { setLinkingSourceId(id); setContextMenu(null); }} onEstablishLink={onEstablishLink} 
                />
            )}
