...
export const StructureFeature: React.FC<StructureFeatureProps> = ({ registry, onRegistryUpdate, onNavigateToWiki }) => {
    ...
    const handleReparent = useCallback((sourceId: string, targetId: string, oldParentId?: string, isReference: boolean = false) => {
        ...
        onRegistryUpdate(prev => {
            if (targetId !== 'root' && GraphIntegrityService.detectCycle(targetId, sourceId, prev)) return prev;
            let next = { ...prev };
            if (!isReference && oldParentId) {
                if (oldParentId === 'root') {
                    const node = next[sourceId];
                    if (node) next[sourceId] = { ...node, tags: ((node as any).tags || []).filter((t: string) => t !== '__is_root__') } as any;
                } else {
            ...
            if (targetId === 'root') {
                const node = next[sourceId];
                if (node) next[sourceId] = { ...node, tags: Array.from(new Set([...((node as any).tags || []), '__is_root__'])) } as any;
                return next;
            }
            ...
            <div className="flex-1 flex overflow-hidden">
                {showSidebar && (
                    <div id="structure-sidebar" className="h-full border-r border-nexus-800 animate-in slide-in-from-left duration-300">
                        <HierarchyExplorer items={Object.values(filteredRegistry)} selectedId={selectedId} onSelect={setSelectedId} onReparent={handleReparent} onDeleteUnit={handleDeleteUnit} />
                    </div>
                )}
                <main className="flex-1 relative overflow-hidden bg-nexus-950">
                    <StructureVisualizer 
                        registry={filteredRegistry} selectedId={selectedId} onSelect={setSelectedId} onAddChild={handleAddChild} onDelete={handleDeleteUnit} 
                        onReparent={handleReparent} onInspect={onNavigateToWiki} onReifyLink={handleReifyLink} onReifyNode={handleReifyNode} onReifyNodeToLink={handleReifyNodeToLink}
                    />
                </main>
            </div>
        </div>
