            ...
            <div className="relative w-full max-w-sm bg-nexus-900 border border-nexus-800 rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 pointer-events-auto">
                <div className="px-8 pt-10 pb-8 text-center border-b border-nexus-800/50">
                    <div className="flex justify-center mb-6">
                        <div className={`w-20 h-20 rounded-[28px] flex items-center justify-center shadow-lg border transition-all ${reified ? 'bg-nexus-accent/10 border-nexus-accent/20 shadow-nexus-accent/10' : 'bg-nexus-900 border-nexus-800 shadow-sm'}`}>
                            {getHeaderIcon()}
                        </div>
                    </div>
                    ...
                    {isL && !reified ? (
                        <div className="flex flex-col items-center">
                            <div className="flex items-center gap-3 text-nexus-muted mb-2">
                                <span className="text-[10px] font-display font-black uppercase tracking-widest truncate max-w-[120px]">{sourceTitle}</span>
                                <ArrowRightLeft size={10} className="text-nexus-accent opacity-50" />
                                <span className="text-[10px] font-display font-black uppercase tracking-widest truncate max-w-[120px]">{targetTitle}</span>
                            </div>
                        ...
                        <>
                            <h3 className="text-2xl font-display font-black text-nexus-text tracking-tight mb-2 truncate px-4">{title}</h3>
                            <div className={`text-[10px] font-mono font-black uppercase tracking-[0.3em] ${reified ? 'text-nexus-accent' : 'text-nexus-muted'}`}>{reified ? 'Reified Logic Unit' : type}</div>
                        </>
                    )}
                </div>
...
const ActionButton = ({ icon: Icon, label, desc, onClick, color = "text-nexus-accent" }: any) => (
    ...
    >
        <div className={`p-3 rounded-2xl bg-nexus-900 border border-nexus-800 group-hover:border-nexus-700 transition-all ${color} shadow-sm`}>
            <Icon size={20} />
        </div>
        <div className="flex-1 min-w-0">
            <div className="text-[14px] font-display font-bold text-nexus-text transition-colors truncate">{label}</div>
            <div className="text-[9px] text-nexus-muted font-black uppercase tracking-widest truncate mt-0.5">{desc}</div>
        </div>
        <ArrowRight size={18} className="text-nexus-muted group-hover:text-nexus-text group-hover:translate-x-1 transition-all shrink-0 opacity-40" />
    </button>
);
