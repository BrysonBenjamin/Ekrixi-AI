...
import { 
    CheckCircle, Trash2, Box, Link2, ChevronRight, Share2, 
    GitBranch, ArrowRight, ShieldAlert 
} from 'lucide-react';
            ...
            <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-nexus-800/50 pb-8">
                <div>
                    ...
                    <p className="text-sm text-nexus-muted mt-2 font-medium">
                        Discovered <span className="text-nexus-text font-bold">{nodes.length}</span> atomic units and <span className="text-nexus-text font-bold">{links.length}</span> potential associations.
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    {totalConflicts > 0 && (
                        <div className="px-4 py-2 bg-amber-500/10 border border-amber-500/30 rounded-xl flex items-center gap-3">
                            <span className="text-[10px] font-display font-black text-amber-500 uppercase tracking-widest">{totalConflicts} Logic Flags Found</span>
                        </div>
                    )}
                    <button 
                        onClick={onCancel}
    ...
    return (
        ...
        `}>
            <div className="flex items-start gap-5">
                ...
                `}>
                    {isRedundant ? <ShieldAlert size={20} /> : isLink ? <Share2 size={20} /> : <Box size={20} />}
                </div>
                
                <div className="flex-1 min-w-0 pt-0.5">
                    <div className="flex items-center gap-3 mb-1.5">
                        <h3 className={`text-sm font-display font-black uppercase tracking-tight truncate ${isRedundant ? 'text-red-400 line-through' : 'text-nexus-text'}`}>
                            {title || 'Untitled'}
                        </h3>
            ...
            {conflict && conflict.status !== 'APPROVED' && (
                <div className="mt-2">
                    ...
                    {conflict.existingPath && (
                        <IntegrityPathTrace path={conflict.existingPath} registry={localRegistry} className="mt-2 pl-5" />
                    )}
                </div>
            )}
        </div>
