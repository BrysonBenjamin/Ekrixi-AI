...
import { 
    Search, Folder, FileText, ChevronDown, ChevronRight, 
    ChevronLeft, Boxes, Share2, Terminal, UserCircle2,
    Move, Link as LinkIcon, AlertCircle, X as CloseIcon, Trash2, Scissors
} from 'lucide-react';
import { NexusObject, isContainer, isLink, NexusType, isReified, NexusGraphUtils } from '../../../types';
...
export const HierarchyExplorer: React.FC<HierarchyExplorerProps> = ({ 
    items, selectedId, onSelect, onReparent, onDeleteReference, onDeleteUnit 
}) => {
            ...
            <React.Fragment key={`${node.id}-${parentId || 'root'}-${isShadowAttachment ? 'shadow' : 'main'}`}>
                ...
                <div className="flex flex-col">
                    ...
                    >
                        ...
                        <button 
                            onClick={(e) => { e.stopPropagation(); setPendingDelete({ id: node.id, parentId: parentId || 'root' }); }}
        ...
        <aside className="w-72 md:w-80 border-r border-nexus-800 bg-nexus-900 flex flex-col overflow-hidden shrink-0 transition-all duration-300 relative z-20 shadow-2xl">
                ...
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-nexus-950/40 backdrop-blur-[2px] animate-in fade-in duration-200">
                    ...
                    <div className="relative bg-nexus-900 border border-nexus-700 rounded-[32px] shadow-2xl overflow-hidden w-full max-w-[280px] animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-5 border-b border-nexus-800/50 bg-nexus-950/30">
                            <div className="text-[9px] font-display font-black text-nexus-muted uppercase tracking-[0.3em] mb-1">Link Protocol</div>
                            <div className="text-sm font-display font-bold text-white truncate">
                                {(registry[pendingDrop.sourceId] as any)?.title} → {(pendingDrop.targetId === 'root' ? 'Origin' : (registry[pendingDrop.targetId] as any)?.title)}
                            </div>
                        </div>
                        {isCycle ? (
                            <div className="p-6 text-center space-y-4">
                                <AlertCircle className="text-red-500 mx-auto" size={24} />
                                <div className="text-[11px] font-display font-black text-red-400 uppercase tracking-widest">Cycle Detected</div>
                                <button onClick={() => setPendingDrop(null)} className="w-full py-3 rounded-2xl bg-nexus-800 text-[10px] font-display font-black uppercase tracking-widest">Dismiss</button>
                            </div>
                        ) : (
                            <div className="p-3 space-y-2">
                                <button onClick={() => handleExecuteDrop(false)} className="w-full flex items-center gap-4 p-3.5 rounded-2xl hover:bg-white/[0.04] transition-all group text-left border border-transparent hover:border-nexus-800">
                                    <div className="p-2 rounded-xl bg-nexus-950 border border-nexus-800 text-nexus-accent group-hover:border-nexus-700"><Move size={18} /></div>
                                    <div><div className="text-[12px] font-display font-bold text-white">Relocate Unit</div><div className="text-[8px] font-mono uppercase tracking-widest text-nexus-muted">Detaches from current parent</div></div>
                                </button>
                                <button onClick={() => handleExecuteDrop(true)} className="w-full flex items-center gap-4 p-3.5 rounded-2xl hover:bg-white/[0.04] transition-all group text-left border border-transparent hover:border-nexus-800">
                                    <div className="p-2 rounded-xl bg-nexus-950 border border-nexus-800 text-nexus-essence group-hover:border-nexus-700"><LinkIcon size={18} /></div>
                                    <div><div className="text-[12px] font-display font-bold text-white">Create Reference</div><div className="text-[8px] font-mono uppercase tracking-widest text-nexus-muted">Multi-parent semantic bind</div></div>
                                </button>
                                <button onClick={() => setPendingDrop(null)} className="w-full py-3 text-[9px] font-display font-black text-nexus-muted uppercase tracking-widest">Abort</button>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {pendingDelete && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-nexus-950/40 backdrop-blur-[2px] animate-in fade-in duration-200">
                    ...
                    <div className="relative bg-nexus-900 border border-nexus-700 rounded-[32px] shadow-2xl overflow-hidden w-full max-w-[280px] animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-5 border-b border-nexus-800/50 bg-nexus-950/30">
                            <div className="text-[9px] font-display font-black text-nexus-muted uppercase tracking-[0.3em] mb-1">Delete Protocol</div>
                            <div className="text-sm font-display font-bold text-white truncate">{(registry[pendingDelete.id] as any)?.title}</div>
                        </div>
                        <div className="p-3 space-y-2">
                            <button 
                                onClick={() => { onDeleteReference?.(pendingDelete.id, pendingDelete.parentId); setPendingDelete(null); }}
                            ...
                            >
                                <div className="p-2 rounded-xl bg-nexus-950 border border-nexus-800 text-amber-500 group-hover:border-nexus-700"><Scissors size={18} /></div>
                                <div><div className="text-[12px] font-display font-bold text-white">Detach Reference</div><div className="text-[8px] font-mono uppercase tracking-widest text-nexus-muted">Removes only this location</div></div>
                            </button>
                            <button 
                                onClick={() => { onDeleteUnit?.(pendingDelete.id); setPendingDelete(null); }}
                                className="w-full flex items-center gap-4 p-3.5 rounded-2xl hover:bg-white/[0.04] transition-all group text-left border border-transparent hover:border-nexus-800"
                            >
                                <div className="p-2 rounded-xl bg-nexus-950 border border-nexus-800 text-red-500 group-hover:border-nexus-700"><Trash2 size={18} /></div>
                                <div><div className="text-[12px] font-display font-bold text-white">Terminate Unit</div><div className="text-[8px] font-mono uppercase tracking-widest text-nexus-muted">Full deletion from registry</div></div>
                            </button>
                            <button onClick={() => setPendingDelete(null)} className="w-full py-3 text-[9px] font-display font-black text-nexus-muted uppercase tracking-widest">Abort</button>
                        </div>
                    </div>
                </div>
            )}
