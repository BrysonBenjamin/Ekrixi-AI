...
export const RefineryDrilldown: React.FC<RefineryDrilldownProps> = ({ 
    registry, focusId, navStack, onNavigateStack, onResetStack, onSelect, onViewModeChange, onReifyLink, onReifyNode, onReifyNodeToLink 
}) => {
    ...
    const visibleNodesRegistry = useMemo(() => {
        ...
        while (queue.length > 0) {
            ...
            (Object.values(registry) as NexusObject[]).forEach(l => {
                if (isLink(l)) {
                    if (l.source_id === id) queue.push({ id: l.target_id, depth: depth + 1, pathType: (pathType === 'focus' || pathType === 'descendant') ? 'descendant' : 'lateral' });
                    else if (l.target_id === id) queue.push({ id: l.source_id, depth: depth + 1, pathType: (pathType === 'focus' || pathType === 'ancestor') ? 'ancestor' : 'lateral' });
                }
            });
        }
        return subRegistry;
        ...
        <div className="w-full h-full flex flex-col bg-nexus-950 relative overflow-hidden">
            ...
            <div className="h-14 border-b border-nexus-800 bg-nexus-900/60 backdrop-blur-xl flex items-center px-6 justify-between shrink-0 z-40 shadow-lg">
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1">
                    ...
                    >
                        <Home size={16} />
                        <span className="text-[10px] font-display font-black uppercase tracking-widest hidden sm:inline">Batch Root</span>
                    </button>
