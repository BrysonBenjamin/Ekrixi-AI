...
export const useStoryStudio = (registry: Record<string, NexusObject>, onCommitBatch: (items: NexusObject[]) => void) => {
    ...
    const activeBook = useMemo(() => activeBookId ? registry[activeBookId] as StoryNote : null, [activeBookId, registry]);

    const handleLoadBook = useCallback((bookId: string) => {
        ...
        const activeNodeIds = Array.from(itemsToLoad.values()).filter(i => !isLink(i)).map(i => i.id);
        allRegistryItems.forEach(item => {
            if (isLink(item)) {
    ...
    const handleSave = useCallback(() => {
        if (stage === 'BLUEPRINT') {
            if (!activeBookId) {
                ...
                if (book) {
                    onCommitBatch([{ ...book, manifesto_data: blocks, last_modified: new Date().toISOString() }]);
                }
            }
        } else {
    ...
    return {
        stage, setStage,
        blocks, setBlocks,
        studioItems, setStudioItems,
        activeBookId, setActiveBookId,
        zoomedChapterId, setZoomedChapterId,
        zoomedSceneId, setZoomedSceneId,
        isCompositeMode, setIsCompositeMode,
        isChapterBlueprintMode, setIsChapterBlueprintMode,
        activeBook,
        handleLoadBook,
        handleCreateNewBook,
        handleSave
