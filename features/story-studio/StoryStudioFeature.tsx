...
export const StoryStudioFeature: React.FC<StoryStudioFeatureProps> = ({ onCommitBatch, registry }) => {
    ...
    const handleImportBlueprint = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (ev) => {
            ...
            } catch (err) { alert("Invalid Blueprint format."); }
        };
        reader.readAsText(file);
    };

    const spineContextLabel = studio.zoomedSceneId || studio.isCompositeMode 
        ? (studio.isCompositeMode ? "Composite Weave" : "Scene Chat") 
        : studio.isChapterBlueprintMode ? "Chapter Blueprint" : studio.zoomedChapterId ? "Chapter Spine" : "Story Spine";
    ...
    return (
        <div className="flex flex-col h-full bg-nexus-950 font-sans text-nexus-text overflow-hidden relative">
            <StudioHeader 
                ...
                isSaveEnabled={studio.stage === 'BLUEPRINT' ? studio.blocks.length > 0 : studio.studioItems.length > 0}
                ...
                <div className="flex-1 h-full relative overflow-hidden">
                    {studio.stage === 'BLUEPRINT' ? (
                        <StudioBookends 
                            ...
                            onFinalize={(b) => { studio.setBlocks(b); studio.setStage('SPINE'); }} 
