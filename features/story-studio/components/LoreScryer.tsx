...
export const LoreScryer: React.FC<LoreScryerProps> = ({ isOpen, registry, onClose, inline }) => {
    ...
    return (
        <aside className={`flex flex-col bg-nexus-900 ${inline ? 'h-full' : 'w-80 border-l border-nexus-800 shadow-2xl z-[60] animate-in slide-in-from-right duration-300'}`}>
            {!inline && (
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-2">
                {items.length === 0 ? (
                    <div className="py-20 text-center opacity-30">
                        ...
                        <p className="text-[9px] font-display font-black uppercase tracking-widest">No Lore Resolved</p>
                    </div>
                ) : (
                    items.map((item: any) => (
                        <div 
                        ...
                        >
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-[11px] font-display font-bold text-nexus-text group-hover:text-nexus-accent truncate">{item.title}</span>
                            </div>
                            <p className="text-[9px] text-nexus-muted italic line-clamp-2 font-serif">"{item.gist}"</p>
                            <div className="mt-3 flex justify-between items-center">
                                <div className="text-[7px] font-mono text-nexus-muted uppercase tracking-widest">DRAG_UNIT</div>
