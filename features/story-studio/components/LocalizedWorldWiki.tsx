        ...
        return (
            <div className="h-full flex flex-col bg-nexus-950 animate-in fade-in slide-in-from-right-4 duration-300">
                <header className="h-14 border-b border-nexus-800 flex items-center px-6 justify-between bg-nexus-900/40">
                    <button onClick={() => setFocusNodeId(null)} className="text-[10px] font-black text-nexus-muted hover:text-nexus-accent uppercase tracking-widest flex items-center gap-2">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar p-8 space-y-8">
                    <div className="space-y-4">
                        <h2 className="text-3xl font-display font-black text-nexus-text uppercase tracking-tight leading-none">{focusedNode.title}</h2>
                        <div className="pl-6 border-l-2 border-nexus-accent/30 py-1">
                             <p className="text-base font-serif italic text-nexus-muted leading-relaxed">"{focusedNode.gist}"</p>
                        </div>
                    </div>
                            ...
                            >
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`text-[8px] font-mono font-black uppercase tracking-widest ${isMentioned ? 'text-nexus-accent' : 'text-nexus-muted'}`}>
                                            {isMentioned ? 'Sync: Active' : node.category_id}
                                        </span>
                                    </div>
                                    <ChevronRight size={14} className="text-nexus-muted group-hover:text-nexus-text group-hover:translate-x-1 transition-all" />
                                </div>
                                <div className="text-sm font-display font-bold text-nexus-text uppercase truncate mb-1">
                                    {node.title}
                                </div>
                                <p className="text-[10px] text-nexus-muted italic line-clamp-1 font-serif">"{node.gist}"</p>
                            </button>
                        );
                    })
                )}
...
const CategoryIcon = ({ cat, reified }: { cat: NexusCategory, reified: boolean }) => {
    ...
    switch (cat) {
        case NexusCategory.LOCATION: return <MapPin size={12} className="text-nexus-accent" />;
        case NexusCategory.CHARACTER: return <Users size={12} className="text-purple-400" />;
