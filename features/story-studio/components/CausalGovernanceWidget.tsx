...
import { ShieldCheck, Plus, X, Edit3, Save, RotateCw } from 'lucide-react';
import { NexusObject, NexusType, NexusCategory, StoryType } from '../../../types';
import { generateId } from '../../../utils/ids';

interface CausalGovernanceWidgetProps {
    items: NexusObject[];
    onUpdate: (items: NexusObject[]) => void;
    onClose: () => void;
}

export const CausalGovernanceWidget: React.FC<CausalGovernanceWidgetProps> = ({ items, onUpdate, onClose }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-nexus-900 border-l border-nexus-800 shadow-2xl overflow-hidden font-sans">
            <header className="h-16 flex items-center px-8 border-b border-nexus-800 bg-amber-500/5 shrink-0">
                <ShieldCheck size={18} className="text-amber-500 mr-3" />
                <h3 className="text-[10px] font-display font-black text-nexus-text uppercase tracking-[0.3em]">Causal Governance</h3>
                ...
                {showNewProtocol && (
                    <div className="bg-nexus-950 border border-amber-500/50 rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                         <div className="flex items-center justify-between mb-4">
                            <span className="text-[9px] font-display font-black text-amber-500 uppercase tracking-widest">Establish Intent</span>
                            <button onClick={() => setShowNewProtocol(false)}><X size={14}/></button>
                         </div>
                         <div className="space-y-4">
                            <input 
                                autoFocus 
                                placeholder="Goal or Theme..." 
                ...
                <div className="space-y-6">
                    {allAuthorNotes.length === 0 && !showNewProtocol && (
                        <div className="py-20 text-center opacity-30 px-10">
                            ...
                            <p className="text-[10px] font-display font-black uppercase tracking-widest">No Active Protocols</p>
                            <p className="text-[9px] font-serif italic mt-2">Establish strategic anchors to govern narrative mass.</p>
                        </div>
                    )}
