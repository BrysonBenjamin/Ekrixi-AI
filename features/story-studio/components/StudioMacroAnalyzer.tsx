...
export const StudioMacroAnalyzer: React.FC<StudioMacroAnalyzerProps> = ({ blocks, onUpdateBlocks, onClose, registry }) => {
    const [isAnalyzing, setIsAnalyzing] = useState(false);
    const [analysisResult, setAnalysisResult] = useState<any>(null);

    const thesis = blocks.find(b => b.type === 'THESIS')?.data.text || "";
    const approach = blocks.find(b => b.type === 'LITERARY_APPROACH')?.data || { archetype: 'THREE_ACT', rationale: '' };
    ...
    const handleSwitchArchetype = (newArch: string) => {
        const nextBlocks = blocks.map(b => 
            b.type === 'LITERARY_APPROACH' 
                ? { ...b, data: { ...b.data, archetype: newArch } } 
                : b
        );
        ...
        <div className="h-full flex flex-col font-sans text-nexus-text">
            <header className="h-20 border-b border-nexus-800 flex items-center justify-between px-10 bg-nexus-950/50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-display font-black text-nexus-text uppercase tracking-tighter">Macro <span className="text-nexus-ruby">Analyzer</span></h2>
                        <p className="text-[10px] font-mono text-nexus-muted uppercase tracking-widest">Theoretical Alignment Engine</p>
                    </div>
                </div>
                <button onClick={onClose} className="p-2 text-nexus-muted hover:text-white transition-colors">
                    <X size={24} />
                </button>
            </header>
                    ...
                    <div className="bg-nexus-950 border border-nexus-800 rounded-[32px] p-8 shadow-xl relative overflow-hidden group">
                         ...
                         <div className="space-y-4 relative z-10">
                             <div className="flex items-center justify-between">
                                <div className="px-3 py-1 bg-nexus-ruby/10 border border-nexus-ruby/30 rounded-full text-[9px] font-black text-nexus-ruby uppercase tracking-widest">{approach.archetype} Protocol</div>
                                <div className="text-[9px] font-mono text-nexus-muted uppercase opacity-40">Thesis Active</div>
                             </div>
                             <h3 className="text-2xl font-serif italic text-nexus-text/90 leading-tight">"{thesis || "No thesis defined."}"</h3>
                             <p className="text-xs text-nexus-muted leading-relaxed border-t border-nexus-800 pt-4 font-serif">
                                <span className="text-nexus-ruby font-bold uppercase text-[9px] tracking-widest block mb-2">Rationale:</span>
                ...
                {!analysisResult ? (
                    <div className="py-12 flex flex-col items-center justify-center text-center space-y-8 animate-in fade-in duration-700">
                        <div className="relative">
                            ...
                            <div className={`p-10 rounded-full border-2 border-dashed border-nexus-800 relative ${isAnalyzing ? 'animate-spin-slow' : ''}`}>
                        ...
                        <button 
                            ...
                            `}
                        >
                            {isAnalyzing ? <RotateCw className="animate-spin" size={18} /> : <Sparkles size={18} />}
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                             <div className="bg-nexus-900 border border-nexus-800 p-8 rounded-[40px] shadow-xl space-y-6">
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-widest">Neural Fit Score</span>
                                    <span className={`text-4xl font-display font-black tracking-tighter ${analysisResult.fitScore > 80 ? 'text-nexus-essence' : 'text-amber-500'}`}>{analysisResult.fitScore}%</span>
                                </div>
                                <div className="w-full h-1.5 bg-nexus-950 rounded-full overflow-hidden border border-nexus-800">
                                    <div className={`h-full transition-all duration-1000 ${analysisResult.fitScore > 80 ? 'bg-nexus-essence shadow-[0_0_15px_var(--essence-color)]' : 'bg-amber-500 shadow-[0_0_15px_#f59e0b]'}`} style={{ width: `${analysisResult.fitScore}%` }} />
                                </div>
                             ...
                             <div className="bg-nexus-900 border border-nexus-800 p-8 rounded-[40px] shadow-xl space-y-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-widest">Causal Friction Points</span>
                                </div>
                                <div className="space-y-4">
                                    {analysisResult.frictionPoints.map((point: string, i: number) => (
                                        <div key={i} className="flex items-start gap-3 p-3 bg-red-500/5 border border-red-500/20 rounded-2xl">
                                             <div className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0 mt-1.5" />
                                             <span className="text-[10px] font-medium text-nexus-text/80 leading-snug">{point}</span>
                                        </div>
                                    ))}
                                </div>
                             </div>
                        ...
                        <div className="space-y-6">
                             <div className="flex items-center gap-3 px-2">
                                ...
                                <span className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-[0.3em]">Alternative Sequence Protocols</span>
                             </div>

                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {analysisResult.alternatives.map((alt: any, i: number) => (
                                    <div key={i} className="group bg-nexus-900 border border-nexus-800 p-6 rounded-[32px] hover:border-nexus-ruby transition-all flex flex-col gap-4">
                                        <div className="flex items-center justify-between">
                                             <div className="text-xs font-display font-black text-nexus-text uppercase tracking-widest">{alt.archetype}</div>
                                             <div className="text-[9px] font-mono text-nexus-essence bg-nexus-essence/10 px-2 py-1 rounded-full border border-nexus-essence/30">+{alt.fidelityGain}% Gain</div>
                                        </div>
            ...
            <footer className="p-8 border-t border-nexus-800 bg-nexus-950/80 backdrop-blur-xl flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4 opacity-70">
                    ...
                    <div>
                        <div className="text-[10px] font-display font-black text-nexus-text uppercase tracking-widest">The Oracle</div>
                        <div className="text-[8px] text-nexus-muted font-mono uppercase tracking-[0.2em] opacity-60">Sequence Optimization v6.2</div>
                    </div>
                </div>
                {analysisResult && (
                    <button 
