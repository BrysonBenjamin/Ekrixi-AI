...
export const ManifestoChatbot: React.FC<ManifestoChatbotProps> = ({ blocks, onUpdateBlocks, registry, onClose }) => {
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
                {messages.length === 0 && (
                    <div className="py-12 text-center space-y-4 opacity-40">
                         ...
                         <p className="text-[10px] font-display font-bold uppercase tracking-widest px-8 text-nexus-text">Define your vision to manifest structural units.</p>
                    </div>
                )}
                {messages.map((m, i) => (
                    <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                        <div className={`max-w-[90%] p-4 rounded-2xl text-[12px] shadow-sm ${m.role === 'user' ? 'bg-nexus-ruby text-white shadow-nexus-ruby/20' : 'bg-nexus-950 border border-nexus-800 text-nexus-text font-serif italic'}`}>
                            {m.text}
                        </div>
                            ...
                            <div className="mt-3 w-full animate-in slide-in-from-left-2 duration-300">
                                <div className="p-4 bg-nexus-ruby/5 border border-nexus-ruby/20 rounded-2xl space-y-4 shadow-inner">
                                    <div className="flex items-center justify-between">
                                        <span className="text-[8px] font-black uppercase text-nexus-ruby tracking-widest">Proposed Logic Units</span>
                                        <Activity size={10} className="text-nexus-ruby animate-pulse" />
                                    </div>
                                    <div className="space-y-2">
                                        {m.action.proposedBlocks.map((b: any, idx: number) => (
                                            <div key={idx} className="flex items-center gap-2 p-2 bg-nexus-900 border border-nexus-800 rounded-lg">
                                                <div className="p-1 bg-nexus-ruby/10 rounded text-nexus-ruby"><Type size={10}/></div>
                                                <span className="text-[9px] font-bold text-nexus-muted truncate">{b.type === 'LITERARY_APPROACH' ? `APPROACH: ${b.data.archetype}` : b.type}</span>
                                            </div>
                                        ))}
                                    </div>
                                    <button 
