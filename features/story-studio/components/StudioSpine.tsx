...
import { 
    ...
    Maximize2, ArrowLeft, TrendingUp, Zap, Library, Wand2, 
    Edit3, RotateCw, Sparkles, ScrollText, Download, Upload, X,
    GitBranch, PenTool, ShieldCheck, Save, AlertTriangle, ArrowRight,
...
export const StudioSpine: React.FC<StudioSpineProps> = ({ 
    ...
    const bookNode = useMemo(() => items.find(i => (i as any).story_type === StoryType.BOOK), [items]);
    const chapters = useMemo(() => items.filter(i => (i as any).story_type === StoryType.CHAPTER).sort((a, b) => (a as any).sequence_index - (b as any).sequence_index), [items]);

    const scenesForZoomedChapter = useMemo(() => {
        if (!zoomedChapterId) return [];
        return items.filter(i => {
            if ((i as any).story_type !== StoryType.SCENE) return false;
            return items.some(l => isLink(l) && l.source_id === zoomedChapterId && l.target_id === i.id);
    ...
    const handleRelinkSequence = (currentList: NexusObject[], baseItems: NexusObject[]) => {
        ...
        const cleanItems = baseItems.filter(i => !((i as any)._type === NexusType.AGGREGATED_HIERARCHICAL_LINK && (i as any).is_reified));
        const sequentialLinks: any[] = [];
        for (let i = 0; i < currentList.length - 1; i++) {
            const source = currentList[i];
            const target = currentList[i + 1];
            sequentialLinks.push({ id: generateId(), _type: NexusType.AGGREGATED_HIERARCHICAL_LINK, is_reified: true, source_id: source.id, target_id: target.id, title: `Sequence Transition`, gist: `Neural transition bridge.`, verb: 'sequenced to', verb_inverse: 'follows', category_id: NexusCategory.STORY, created_at: now, last_modified: now, link_ids: [], internal_weight: 1.0, total_subtree_mass: 0 });
        }
    ...
    const handleSynthesizeScenesForChapter = async (chapterId: string) => {
        ...
        try {
            ...
            const hierarchyLinks = generatedScenes.map(sc => ({
                id: generateId(), _type: NexusType.HIERARCHICAL_LINK, source_id: chapterId, target_id: sc.id, verb: 'contains', hierarchy_type: HierarchyType.PARENT_OF, created_at: now, last_modified: now, link_ids: []
            }));
    ...
    const handleDeleteBeat = (id: string) => {
        const nextItems = items.filter(item => item.id !== id && (!isLink(item) || (item.source_id !== id && item.target_id !== id)))
            .map(item => (isContainer(item) && item.children_ids.includes(id)) ? { ...item, children_ids: item.children_ids.filter(cid => cid !== id) } : item);
        onUpdate(nextItems as any);
    ...
    const handleAutoFillMetadata = async (id: string) => {
        ...
        try {
            ...
            const result = await StudioSpineAgent.autoFillMetadata(target, prev, next, blocks, autoFillPrompt);
    ...
    const handleNeuralFill = async (id: string) => {
        ...
        try {
            const list = zoomedChapterId ? scenesForZoomedChapter : chapters;
            const idx = list.findIndex(c => c.id === id);
            const result = await StudioSpineAgent.completeDraft(target, idx > 0 ? list[idx - 1] : null, idx < list.length - 1 ? list[idx + 1] : null, blocks);
            handleUpdateBeat(id, { gist: result.gist, prose_content: result.content });
        } catch (err) { console.error(err); } finally { setIsFillingId(null); }
    };

    const handleAddManualBeat = () => {
        const now = new Date().toISOString();
        const chId = generateId();
        const newChapter: any = { id: chId, _type: NexusType.STORY_NOTE, story_type: StoryType.CHAPTER, title: "New Chapter Bead", gist: "...", sequence_index: chapters.length + 1, tension_level: 50, status: NarrativeStatus.OUTLINE, category_id: NexusCategory.STORY, created_at: now, last_modified: now, link_ids: [], children_ids: [], internal_weight: 1.0, total_subtree_mass: 0 };
        const nextItems = [...items, newChapter];
        if (bookNode) {
            nextItems.push({ id: generateId(), _type: NexusType.HIERARCHICAL_LINK, source_id: bookNode.id, target_id: chId, verb: 'contains', hierarchy_type: HierarchyType.PARENT_OF, created_at: now, last_modified: now, link_ids: [] } as any);
        }
        onUpdate(nextItems as any);
        setEditingId(chId);
    };
    ...
    const handleAddScene = (chapterId: string) => {
        ...
        const newScene: any = { id: sceneId, _type: NexusType.STORY_NOTE, story_type: StoryType.SCENE, title: "New Scene Bead", gist: "...", status: NarrativeStatus.OUTLINE, sequence_index: scenesForZoomedChapter.length + 1, tension_level: 50, category_id: NexusCategory.STORY, created_at: now, last_modified: now, link_ids: [], children_ids: [], internal_weight: 1.0, total_subtree_mass: 0 };
        onUpdate([...items, newScene, { id: generateId(), _type: NexusType.HIERARCHICAL_LINK, source_id: chapterId, target_id: sceneId, verb: 'contains', hierarchy_type: HierarchyType.PARENT_OF, created_at: now, last_modified: now, link_ids: [] } as any);
        ...
        return (
            <div className="h-full flex flex-col items-center justify-center p-12 bg-nexus-950 animate-in fade-in duration-1000">
                <div className="relative mb-12">
                    ...
                    <div className="relative w-40 h-40 rounded-[56px] bg-nexus-900 border border-nexus-ruby/20 flex items-center justify-center shadow-2xl"><GitBranch size={64} className="text-nexus-ruby" /></div>
                </div>
                    ...
                    <div className="flex gap-4 justify-center">
                        <button onClick={async () => {
                            ...
                            try {
                                ...
                                const book: any = { id: newBookId, _type: NexusType.STORY_NOTE, story_type: StoryType.BOOK, title: "Untitled Manuscript", gist: "Active Manuscript", category_id: NexusCategory.STORY, created_at: new Date().toISOString(), last_modified: new Date().toISOString(), link_ids: [], children_ids: generatedChapters.map(c => c.id), internal_weight: 1.0, total_subtree_mass: 0 };
                                const finalItems = handleRelinkSequence(generatedChapters, [book, ...generatedChapters]);
                                onCommitBatch(finalItems as NexusObject[]);
                            } catch (err) { console.error(err); } finally { setIsSynthesizing(false); setSynthStatus(null); }
                        }} className="px-12 py-5 bg-nexus-ruby text-white rounded-3xl font-display font-black text-[11px] uppercase tracking-widest shadow-2xl flex items-center gap-4 hover:brightness-110 active:scale-95 transition-all"><Sparkles size={18} /> Run Architect</button>
                        <button onClick={handleAddManualBeat} className="px-12 py-5 bg-nexus-900 border border-nexus-800 text-nexus-muted rounded-3xl font-display font-black text-[11px] uppercase tracking-widest hover:text-white transition-all flex items-center gap-4"><Plus size={18} /> Manual Entry</button>
                    </div>
                </div>
            </div>
        );
        ...
        return (
            <div className="h-full flex flex-col bg-nexus-950">
                 <header className="h-14 flex items-center justify-between px-8 border-b border-nexus-800 bg-nexus-900/40">
                    <button onClick={() => { onSetZoomedSceneId(null); onSetCompositeMode(false); }} className="flex items-center gap-2 text-nexus-muted hover:text-nexus-accent text-[10px] font-black uppercase tracking-widest transition-all">
                        <ArrowLeft size={16} /> Back to Chapter Directory
                    </button>
                    <div className="flex items-center gap-3">
                         <span className="px-3 py-1 bg-nexus-arcane/10 border border-nexus-arcane/30 rounded-full text-[8px] font-black text-nexus-arcane uppercase tracking-widest">{isCompositeMode ? 'Composite Chapter Stack' : 'Focused Content Story Note'}</span>
        ...
        <div className="h-full flex flex-col bg-nexus-950 overflow-hidden animate-in slide-in-from-bottom-4 duration-700">
            <header className="h-20 flex items-center justify-between px-10 border-b border-nexus-800 bg-nexus-900/20 shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={zoomedChapterId ? () => handleAddScene(zoomedChapterId) : handleAddManualBeat} className={`flex items-center gap-2 px-4 py-1.5 border hover:text-white transition-all rounded-full text-[9px] font-black uppercase tracking-widest ${zoomedChapterId ? 'bg-nexus-arcane/10 border-nexus-arcane/30 text-nexus-arcane hover:bg-nexus-arcane' : 'bg-nexus-950 border border-nexus-800 hover:border-nexus-ruby text-nexus-text'}`}><Plus size={14} /> {zoomedChapterId ? 'Append Scene' : 'Append Chapter'}</button>
                </div>
            </header>
                ...
                {zoomedChapterId && currentList.length === 0 && !isChapterManifestoMode ? (
                    ...
                    <div className="max-w-4xl mx-auto">
                        <ManifestoForge 
                            ...
                            canSynthesize={(chapterManifestos[zoomedChapterId] || []).some(b => b.type === 'THESIS')}
                            onCommitUnit={(unit) => {
                    ...
                    currentList.map((ch: any, idx: number) => (
                        <div key={ch.id} className="group relative flex gap-8 animate-in slide-in-from-left-4 duration-500">
                            <div className="flex flex-col items-center shrink-0">
                                <div className={`w-14 h-14 rounded-2xl bg-nexus-900 border border-nexus-800 flex items-center justify-center font-black text-lg transition-all ${zoomedChapterId ? 'text-nexus-arcane group-hover:border-nexus-arcane' : 'text-nexus-ruby group-hover:border-nexus-ruby'}`}>{ch.sequence_index}</div>
                                <div className="flex flex-col gap-1 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleMoveBeat(ch.id, 'up')} disabled={idx === 0} className="p-1 text-nexus-muted hover:text-white disabled:opacity-20"><ArrowUp size={14}/></button>
                                    <button onClick={() => handleMoveBeat(ch.id, 'down')} disabled={idx === currentList.length - 1} className="p-1 text-nexus-muted hover:text-white disabled:opacity-20"><ArrowDown size={14}/></button>
                                </div>
                                <div className="flex-1 w-px bg-nexus-800 my-3 group-last:hidden" />
                            </div>
                            <div className={`flex-1 bg-nexus-900/40 border rounded-[32px] p-8 transition-all shadow-sm group-hover:border-opacity-50 min-w-0 ${editingId === ch.id ? (zoomedChapterId ? 'border-nexus-arcane bg-nexus-arcane/5 ring-1 ring-nexus-arcane' : 'border-nexus-ruby bg-nexus-ruby/5 ring-1 ring-nexus-ruby') : 'border-nexus-800 hover:bg-nexus-900/60'}`}>
                                {editingId === ch.id ? (
                                    <div className="space-y-6 w-full animate-in zoom-in-95 duration-200">
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between px-2">
                                                <label className="text-[10px] font-black uppercase text-nexus-muted tracking-widest">Beat Metadata</label>
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[8px] font-mono text-nexus-muted uppercase opacity-40">Auto-fill Logic</span>
                                                    <Activity size={10} className="text-nexus-ruby animate-pulse" />
                                                </div>
                                            </div>
                                            <input autoFocus value={ch.title} onChange={(e) => handleUpdateBeat(ch.id, { title: e.target.value })} className="w-full bg-nexus-950 border border-nexus-800 rounded-xl px-4 py-3 text-lg font-display font-bold uppercase outline-none focus:border-nexus-ruby text-nexus-text" placeholder="Title..." />
                                            <textarea value={ch.gist} onChange={(e) => handleUpdateBeat(ch.id, { gist: e.target.value })} className="w-full h-24 bg-nexus-950 border border-nexus-800 rounded-xl px-4 py-3 text-sm font-serif italic outline-none focus:border-nexus-ruby resize-none no-scrollbar text-nexus-text" placeholder="Description..." />
                                        </div>

                                        <div className="bg-nexus-950 border border-nexus-800 rounded-2xl p-4 space-y-4">
                                            <div className="flex items-center gap-3">
                                                <Wand2 size={14} className="text-nexus-ruby" />
                                                <span className="text-[9px] font-black uppercase tracking-widest text-nexus-text">Contextual Auto-fill</span>
                                            </div>
                                            <div className="flex gap-2">
                                                ...
                                                >
                                                    {isAutoFilling ? <RotateCw size={12} className="animate-spin" /> : <Sparkles size={12} />}
                                                    Run Fill
                                                </button>
                                            </div>
                                        ...
                                        <div className="flex justify-end gap-3 pt-2">
                                            <button onClick={() => handleDeleteBeat(ch.id)} className="p-3 text-red-500 hover:bg-red-500/10 rounded-xl transition-colors"><Trash2 size={18}/></button>
                                            <button onClick={() => setEditingId(null)} className={`px-6 py-2 text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg ${zoomedChapterId ? 'bg-nexus-arcane' : 'bg-nexus-ruby'}`}><Check size={14}/> Complete</button>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="w-full">
                                        <div className="flex items-center justify-between mb-4 gap-4">
                                            <h3 className="text-xl font-display font-black text-nexus-text uppercase tracking-tight truncate text-nexus-text">{ch.title}</h3>
                                            <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        {!zoomedChapterId && (
                                            <div className="space-y-4 border-t border-nexus-800/50 pt-8">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    ...
                                                    <button onClick={() => onSetZoomedChapterId(ch.id)} className="px-4 py-2 border border-dashed border-nexus-800 rounded-xl text-[10px] text-nexus-muted hover:bg-nexus-arcane/5 hover:text-nexus-arcane transition-all flex items-center gap-2"><Plus size={10}/> Add Scene</button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-end justify-center p-8 bg-nexus-950/60 backdrop-blur-md animate-in fade-in duration-300">
                    ...
                    <div className="relative w-full max-w-4xl bg-nexus-900 border border-nexus-800 rounded-[48px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-500">
                        <header className="h-20 flex items-center justify-between px-10 border-b border-nexus-800">
                            <div className="flex items-center gap-4"><ShieldAlert size={24} className={auditResult?.status === 'NEEDS_REFACTOR' ? 'text-red-500' : 'text-nexus-essence'} /><div><h3 className="text-xl font-display font-black text-nexus-text uppercase tracking-tight text-nexus-text">Structural <span className="text-nexus-ruby">Audit</span></h3><p className="text-[10px] font-mono text-nexus-muted uppercase tracking-widest">Neural Analysis Complete</p></div></div>
                            <button onClick={() => setShowAudit(false)} className="p-2 text-nexus-muted hover:text-white"><X size={24} /></button>
                        </header>
                        <div className="p-10 space-y-8 overflow-y-auto no-scrollbar max-h-[70vh]">
                            {isAuditing ? <div className="py-20 flex flex-col items-center gap-4"><RotateCw className="animate-spin text-nexus-ruby" size={48} /><span className="text-[10px] font-black uppercase tracking-widest text-nexus-muted">Evaluating Sequence Fidelity...</span></div> : auditResult && (
                                <><div className={`p-8 rounded-[32px] border ${auditResult.status === 'NEEDS_REFACTOR' ? 'bg-red-500/5 border-red-500/30' : 'bg-nexus-essence/5 border-nexus-essence/30'}`}><div className="flex items-center gap-3 mb-4">{auditResult.status === 'NEEDS_REFACTOR' ? <AlertTriangle className="text-red-500" size={20} /> : <Check className="text-nexus-essence" size={20} />}<span className={`text-[10px] font-black uppercase tracking-widest ${auditResult.status === 'NEEDS_REFACTOR' ? 'text-red-400' : 'text-nexus-essence'}`}>Status: {auditResult.status}</span></div><p className="text-lg font-serif italic text-nexus-text leading-relaxed">"{auditResult.critique}"</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{auditResult.alternatives.map((alt: any, i: number) => (<div key={i} className="p-8 bg-nexus-950 border border-nexus-800 rounded-[32px] hover:border-nexus-ruby/50 transition-all group"><h4 className="text-sm font-display font-black text-nexus-text uppercase tracking-widest mb-4 flex items-center gap-2"><Sparkles size={14} className="text-nexus-ruby" /> Alternative {i + 1}: {alt.name}</h4><p className="text-xs text-nexus-muted leading-relaxed font-serif">{alt.rationale}</p><button className="mt-6 text-[9px] font-black uppercase tracking-[0.2em] text-nexus-muted group-hover:text-nexus-ruby transition-colors flex items-center gap-2">Explore Potential <ChevronRight size={12}/></button></div>))}</div></>
                            )}
                        </div>
                    </div>
                </div>
...
const TensionIndicator = ({ value }: { value: number }) => <div className="flex items-center gap-3 px-3 py-1.5 bg-nexus-950 border border-nexus-800 rounded-xl"><div className="group/tension relative flex items-center gap-3 cursor-help"><TrendingUp size={12} className="text-nexus-ruby" /><div className="w-20 h-1 bg-nexus-900 rounded-full overflow-hidden border border-nexus-800"><div className="h-full bg-nexus-ruby shadow-[0_0_8px_rgba(225,29,72,0.6)] transition-all duration-1000" style={{ width: `${value}%` }} /></div><span className="text-[10px] font-mono font-black text-nexus-muted">{value}%</span></div></div>;
