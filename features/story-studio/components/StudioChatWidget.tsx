...
import { X, Send, Bot, User, Sparkles, RotateCw, Activity, ArrowUpRight, Box, Library, ChevronRight, BookOpen, ShieldCheck, Search } from 'lucide-react';
import { NexusObject, StoryType, NexusType, NexusCategory, NarrativeStatus, HierarchyType } from '../../../types';
...
export const StudioChatWidget: React.FC<StudioChatWidgetProps> = ({ isOpen, onClose, items, onUpdateItems, blocks, inline }) => {
    ...
    const handleExecuteAction = (type: string, data: any) => {
        ...
        if (type === 'CHAPTERS') {
            data.forEach((ch: any) => {
                ...
                if (book) {
                    nextItems.push({
                        id: generateId(), _type: NexusType.HIERARCHICAL_LINK, source_id: book.id, target_id: newId, verb: 'contains', hierarchy_type: HierarchyType.PARENT_OF, created_at: now, last_modified: now, link_ids: []
                    } as any);
                }
            });
            ...
            data.forEach((prot: any) => {
                ...
                nextItems.push({
                    id: pId, _type: NexusType.SIMPLE_NOTE, title: prot.title, gist: prot.gist, category_id: NexusCategory.META, is_author_note: true, created_at: now, last_modified: now, link_ids: [], internal_weight: 1.0, total_subtree_mass: 0
                } as any);
                if (prot.targetChapterIds) {
                    prot.targetChapterIds.forEach((chId: string) => {
                        if (items.some(i => i.id === chId)) {
                             nextItems.push({ id: generateId(), _type: NexusType.SEMANTIC_LINK, source_id: pId, target_id: chId, verb: 'governs', created_at: now, last_modified: now, link_ids: [] } as any);
                        }
                    });
                }
            });
        ...
        setMessages(prev => [...prev, { role: 'assistant', text: "Logical changes committed to story spine." }]);
    };

    return (
        <div className={`flex flex-col h-full bg-nexus-900 ${inline ? '' : 'w-96 border-l border-nexus-800 shadow-2xl animate-in slide-in-from-right duration-300'}`}>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
                {messages.length === 0 && (
                    <div className="py-12 text-center space-y-4 opacity-40">
                         ...
                         <p className="text-[10px] font-display font-bold uppercase tracking-widest px-8 text-nexus-text">Awaiting Strategic Directives...</p>
                    </div>
                )}
                {messages.map((m, i) => (
                    <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                        {m.context && m.context.length > 0 && (
                            <div className="flex flex-wrap gap-1 mb-1">
                                {m.context.map((n: any) => (
                                    <span key={n.id} className="px-2 py-0.5 bg-nexus-accent/10 border border-nexus-accent/30 rounded text-[7px] font-bold text-nexus-accent uppercase">@{n.title}</span>
                                ))}
                            </div>
                        )}
                        <div className={`max-w-[95%] p-4 rounded-2xl text-[12px] shadow-sm ${m.role === 'user' ? 'bg-nexus-ruby border border-nexus-ruby/20 text-white' : 'bg-nexus-950 border border-nexus-800 text-nexus-text'}`}>
                            {m.text}
                        </div>
                        {m.action && (
            ...
            >
                <div className="flex flex-wrap gap-2 mb-3">
                    {contextNodes.map(n => (
                        <div key={n.id} className="flex items-center gap-2 px-3 py-1 bg-nexus-accent/10 border border-nexus-accent/30 rounded-full text-[9px] font-bold text-nexus-accent">
                            {n.title} <button onClick={() => setContextNodes(contextNodes.filter(cn => cn.id !== n.id))}><X size={10}/></button>
                        </div>
                    ))}
                    {contextNodes.length > 0 && <button onClick={() => setContextNodes([])} className="text-[9px] text-nexus-muted hover:text-nexus-text uppercase font-black px-2">Clear</button>}
                </div>
                <div className="relative">
                    ...
                    />
                    <button onClick={handleSend} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-nexus-ruby text-white rounded-xl shadow-lg active:scale-95 transition-transform"><Send size={14} /></button>
                </div>
                <p className="mt-2 text-[8px] text-center text-nexus-muted uppercase tracking-widest opacity-40">Drag lore units to chat for deep contextual analysis</p>
            </div>
        </div>
    );
};
...
const ActionButton = ({ icon: Icon, label, onClick }: any) => (
    <button onClick={onClick} className="w-full py-2 bg-nexus-essence/10 border border-nexus-essence/30 text-nexus-essence rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-nexus-essence hover:text-black transition-all flex items-center justify-center gap-2 shadow-lg">
        <Icon size={12} /> {label}
    </button>
);
