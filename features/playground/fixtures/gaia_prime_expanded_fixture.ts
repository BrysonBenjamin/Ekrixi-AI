import { NexusObject, NexusType, NexusCategory, ContainmentType, DefaultLayout, HierarchyType } from '../../../types';
...
export const getGaiaPrimeExpandedBatch = (): NexusObject[] => {
    ...
    const createNode = (data: any): string => {
        const node = { ...data, created_at: timestamp, last_modified: timestamp, link_ids: [] as string[], internal_weight: 1.0, total_subtree_mass: 0, is_ghost: false };
    ...
    const createLink = (sourceId: string, targetId: string, type: NexusType, verb: string) => {
        ...
        if (type === NexusType.HIERARCHICAL_LINK) (link as any).hierarchy_type = HierarchyType.PARENT_OF;
    ...
    // 2. Regions
    const regions = [
        { title: "Aetheria", gist: "Floating continent of eternal winds.", cat: NexusCategory.LOCATION },
        { title: "Abyssa Trenches", gist: "Deep-sea bioluminescent civilizations.", cat: NexusCategory.LOCATION },
        { title: "Obsidian Maw", gist: "Geothermal canyons where Gaia's blood (magma) is exposed.", cat: NexusCategory.LOCATION },
        { title: "Cloud-Garden Ichor", gist: "Aerial botanical clusters that filter toxin-rain.", cat: NexusCategory.LOCATION }
    ];
    ...
    createNode({
        ...
        tags: ["pilot"],
        prose_content: "Elara was the first to realize that Gaia's tectonic shifts were actually long-wave communications."
    });
