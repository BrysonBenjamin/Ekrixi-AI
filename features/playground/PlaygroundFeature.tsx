...
import { FlaskConical, Hammer, Database, Sparkles, ChevronRight, Package, Terminal, Box, BookOpen, PenTool, Globe } from 'lucide-react';
...
export const PlaygroundFeature: React.FC<PlaygroundFeatureProps> = ({ onSeedRefinery, onSeedRegistry, onSeedManifesto }) => {
    return (
            ...
            <div className="max-w-5xl mx-auto space-y-16">
                ...
                <header className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-nexus-muted font-mono uppercase tracking-[0.4em]">Development & Vibe-Coding Sandbox v1.2</p>
                        </div>
                    </div>
                </header>
                ...
                <section className="space-y-8">
                    <div className="flex items-center gap-4 border-b border-nexus-800 pb-4">
                        ...
                        <h2 className="text-sm font-display font-black text-nexus-muted uppercase tracking-[0.2em]">Dramaturgical Injection System</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <FixtureCard 
                            ...
                            icon={BookOpen}
                            theme="ruby"
                            onSeed={() => onSeedRefinery(getCompletedManuscriptBatch(), "GAIA_RESONANCE_POLISHED")}
                        />
                        ...
                        <FixtureCard 
                            ...
                            icon={Database}
                            onSeed={() => onSeedRegistry(getGaiaPrimeExpandedBatch().reduce((acc, cur) => ({...acc, [cur.id]: cur}), {}))}
                ...
                <section className="space-y-8">
                    ...
                    <div className="p-8 bg-nexus-900/50 border border-nexus-800 rounded-[32px] flex flex-col items-center justify-center text-center space-y-6">
                        ...
                        <p className="text-xs text-nexus-muted font-serif italic max-w-sm">"Development space for future neural weights and reification testing."</p>
...
const FixtureCard = ({ title, desc, itemsCount, category, onSeed, actionLabel = "Send to Refinery", variant = "refinery", icon: Icon = Sparkles, theme = "cyan" }: any) => {
    const accentClass = theme === 'ruby' ? 'text-nexus-ruby' : 'text-nexus-accent';
    const borderAccent = theme === 'ruby' ? 'border-nexus-ruby/30 hover:border-nexus-ruby' : 'border-nexus-accent/30 hover:border-nexus-accent';
    const bgAccent = theme === 'ruby' ? 'bg-nexus-ruby/10' : 'bg-nexus-accent/10';

    return (
        <div className={`group relative p-8 bg-nexus-900 border rounded-[40px] transition-all duration-500 hover:translate-y-[-4px] shadow-xl hover:shadow-2xl ${variant === 'registry' ? 'border-nexus-essence/30 hover:border-nexus-essence' : 'border-nexus-800 hover:border-nexus-accent'} ${theme === 'ruby' ? 'hover:border-nexus-ruby/50' : ''}`}>
            <div className="flex items-start justify-between mb-8">
                <div className="space-y-1">
                    <span className={`text-[8px] font-mono font-black px-2 py-0.5 rounded-full border uppercase tracking-widest ${variant === 'registry' ? 'bg-nexus-essence/10 border-nexus-essence/30 text-nexus-essence' : `${bgAccent} border-transparent ${accentClass}`}`}>
                        {category}
                    </span>
