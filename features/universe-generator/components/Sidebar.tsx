...
export const Sidebar: React.FC<SidebarProps> = ({ sessions, activeId, onSelect, onCreate, onDelete }) => {
    return (
                ...
                {sessions.map(session => (
                    ...
                    >
                        <div className="flex items-center gap-3 overflow-hidden min-w-0">
                            <div className="w-1.5 h-1.5 rounded-full shrink-0 transition-all duration-300" style={{ backgroundColor: activeId === session.id ? 'var(--accent-color)' : 'transparent' }}></div>
                            <span className={`text-sm truncate font-medium block w-full ${activeId === session.id ? 'font-bold' : ''}`}>
                                {session.title}
                            </span>
                        </div>
                        <button 
