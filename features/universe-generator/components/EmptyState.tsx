            ...
            <div className="w-full max-w-4xl z-10 flex flex-col gap-16 animate-in fade-in slide-in-from-bottom-12 duration-1000 items-center">
                ...
                <div className="space-y-6 text-center">
                    <div className="flex items-center justify-center gap-3 mb-4">
                        <Logo size={48} />
                        <span className="text-xs font-display font-black text-nexus-muted tracking-[0.4em] uppercase">Ekrixi AI Core</span>
                    </div>
                    ...
                    <p className="text-nexus-muted text-lg max-w-2xl mx-auto font-serif italic font-medium">
                        Enter the weaving protocol to manifest legendary timelines and divine hierarchies with Ekrixi AI.
                    </p>
                </div>
                ...
                <div className="flex flex-wrap gap-4 justify-center w-full max-w-4xl px-4">
                    <SuggestionChip 
                        icon={MapIcon} 
                        label="MAP_REGISTRY" 
                        onClick={() => onSend("Construct a logic map for a city built on the rotating rings of a dying sun.")} 
                    />
                    <SuggestionChip 
                        icon={Users} 
                        label="INIT_CHARACTER" 
                        onClick={() => onSend("Define a protagonist who exists across three parallel timelines simultaneously.")} 
                    />
                    <SuggestionChip 
                        icon={Shield} 
                        label="SECURE_FACTION" 
                        onClick={() => onSend("Establish a secret organization that monitors the dreams of the planetary consciousness.")} 
                    />
                </div>

                <div className="mt-12 flex flex-col items-center gap-3 opacity-30">
                    <Sparkles size={20} className="text-nexus-accent" />
                    <p className="text-[10px] font-mono font-black uppercase tracking-[0.6em] text-nexus-muted">Ekrixi Kernel v5.2 Active // Registry Scry Enabled</p>
                </div>
