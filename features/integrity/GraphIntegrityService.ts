...
export const GraphIntegrityService = {
   ...
   */
  detectCycle: (sourceId: string, targetId: string, registry: Record<string, NexusObject>): boolean => {
  ...
  analyzeLinkIntegrity: (
    ...
    const isProposedHierarchical = proposedType && (
    ...
    const isProposedReified = proposedType && (
    ...
    while (queue.length > 0) {
      ...
      if (id === targetId && dist > 0) {
        const onlyHierarchicalInPath = types.every(t => 
   ...
   */
  getRegistryIntegrityMap: (registry: Record<string, NexusObject>): Record<string, IntegrityReport> => {
