...
export const IntegrityPathTrace: React.FC<IntegrityPathTraceProps> = ({ path, registry, className = "" }) => {
  ...
  return (
    <div className={`flex items-center gap-2 overflow-x-auto no-scrollbar py-1 ${className}`}>
      ...
      <span className="text-[8px] font-mono text-nexus-muted/60 uppercase whitespace-nowrap">Path:</span>
      <div className="flex items-center gap-1.5">
        {path.map((nodeId, idx) => {
          const node = registry[nodeId];
          const label = (node as any)?.title || (node as any)?.verb || nodeId.slice(0, 4);
          return (
            <React.Fragment key={`${nodeId}-${idx}`}>
              ...
              {idx < path.length - 1 && <ArrowRight size={8} className="text-nexus-muted/40 shrink-0" />}
            </React.Fragment>
          );
        })}
      </div>
