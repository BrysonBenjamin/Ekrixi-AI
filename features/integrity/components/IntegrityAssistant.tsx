...
import { 
    ShieldAlert, X, ChevronRight, Trash2, Share2, 
    Sparkles, Info, Eye, Zap, BrainCircuit, History,
    CheckCircle2, AlertTriangle, Fingerprint, Target, Compass, ArrowLeft
} from 'lucide-react';
...
interface IntegrityAssistantProps {
    ...
    onFocusAnomaly: (data: { linkId: string, path?: string[], mode: 'CENTER' | 'DRILL' } | null) => void;
}

export const IntegrityAssistant: React.FC<IntegrityAssistantProps> = ({ 
    isOpen, onClose, registry, onResolve, onFocusAnomaly 
}) => {
        ...
        <div className="fixed inset-y-0 right-0 w-[420px] bg-nexus-900 border-l border-nexus-800 shadow-[-20px_0_60px_rgba(0,0,0,0.5)] z-[600] flex flex-col animate-in slide-in-from-right duration-500 font-sans">
            ...
            <header className="h-20 border-b border-nexus-800 flex items-center justify-between px-8 shrink-0 bg-nexus-900/80 backdrop-blur-xl">
                <div className="flex items-center gap-4">
                    {activeAnomaly ? (
                        <button 
                            onClick={() => { setSelectedAnomalyId(null); onFocusAnomaly(null); }}
                ...
                <button onClick={() => { onFocusAnomaly(null); onClose(); }} className="p-2 text-nexus-muted hover:text-nexus-text transition-colors">
                    <X size={20} />
                </button>
            </header>
            ...
            <main className="flex-1 overflow-y-auto p-6 no-scrollbar space-y-6">
                {anomalies.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-40 px-10">
                        ...
                        <h3 className="text-lg font-display font-black text-nexus-text uppercase tracking-widest mb-2">Registry Unified</h3>
                        <p className="text-xs text-nexus-muted leading-relaxed font-serif italic">"The Chronicler finds no logic drift in the current sector. Your knowledge architecture is structurally sound."</p>
                    </div>
                ) : activeAnomaly ? (
                    /* Detail View */
                        ...
                        <div className="p-8 bg-nexus-950 border border-nexus-800 rounded-[40px] shadow-2xl relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div className="flex-1">
                                    <h3 className="text-xs font-display font-black text-nexus-text uppercase tracking-widest">Chronicler's Audit</h3>
                                    <div className="text-[8px] font-mono text-red-500 uppercase mt-0.5 font-bold">{activeAnomaly.info.status} DETECTED</div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-2 mb-8">
                                <button 
                                    onClick={() => onFocusAnomaly({ linkId: activeAnomaly.link.id, path: activeAnomaly.info.path, mode: 'CENTER' })}
                                ...
                                <button 
                                    onClick={() => onFocusAnomaly({ linkId: activeAnomaly.link.id, path: activeAnomaly.info.path, mode: 'DRILL' })}
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                <DecisionButton 
                                    ...
                                    color="text-red-500" 
                                    bg="bg-red-500/10"
                                    onClick={() => { onResolve(activeAnomaly.link.id, 'DELETE'); setSelectedAnomalyId(null); }}
                                />
                                <DecisionButton 
                                    icon={Share2} 
                                    label="Reify Logic" 
                                    ...
                                    onClick={() => { onResolve(activeAnomaly.link.id, 'REIFY'); setSelectedAnomalyId(null); }}
                                />
                                <DecisionButton 
                                    icon={Eye} 
                                    label="Sequester" 
                            ...
                            {anomalies.map(({ link, info }) => (
                                ...
                                >
                                    <div className="flex items-center justify-between mb-2">
                                        <span className={`text-[8px] font-mono font-black px-2 py-0.5 rounded-full border ${info.status === 'REDUNDANT' ? 'bg-red-500/10 border-red-500/30 text-red-500' : 'bg-amber-500/10 border-amber-500/30 text-amber-500'}`}>
                                            {info.status}
                                        </span>
            ...
            <footer className="p-8 border-t border-nexus-800 bg-nexus-950 flex items-center gap-4 shrink-0">
                ...
                <div>
                    ...
                    <div className="text-[8px] text-nexus-muted font-mono uppercase tracking-[0.2em] opacity-60">Logic Engine v4.8</div>
                </div>
            </footer>
        </div>
    );
...
const DecisionButton = ({ icon: Icon, label, desc, color, bg, onClick }: any) => (
    ...
    >
        <div className={`p-2.5 rounded-xl ${bg} ${color} transition-transform group-hover:scale-110 shadow-sm`}>
            <Icon size={18} />
        </div>
        <div className="flex-1 min-w-0">
            <div className="text-[12px] font-display font-bold text-nexus-text transition-colors">{label}</div>
            <div className="text-[8px] font-mono uppercase tracking-widest text-nexus-muted mt-0.5">{desc}</div>
        </div>
    </button>
);
