...
const TEMPLATES: TemplateInfo[] = [
    { id: 'THREE_ACT', title: 'Three-Act Structure', desc: 'The classic Setup, Confrontation, and Resolution arc.', icon: Anchor },
    { id: 'HERO_JOURNEY', title: 'The Hero\'s Journey', desc: 'Monomyth approach based on Joseph Campbell\'s theory.', icon: Sword },
    { id: 'SAVE_CAT', title: 'Save the Cat!', desc: '15 beats focusing on pacing and audience engagement.', icon: Trophy },
    { id: 'FICHTEAN', title: 'Fichtean Curve', desc: 'High-tension structure moving from crisis to crisis.', icon: Target },
];

export const ManuscriptAnalyzerFeature: React.FC<ManuscriptAnalyzerFeatureProps> = ({ onCommitBatch }) => {
    ...
    const handleGenerateBlueprint = async () => {
        ...
        try {
            ...
            const theoryResponse = await ai.models.generateContent({
                            ...
                            units: {
                                ...
                                items: {
                                    ...
                                    properties: {
                                        ...
                                        type: { type: Type.STRING, description: "ARC, CHAPTER, SCENE, or THEME" },
                                        gist: { type: Type.STRING },
                                        theory_beat: { type: Type.STRING, description: "Which part of the theory this unit represents (e.g., 'Incite Incident')." },
                                        parent_title: { type: Type.STRING, description: "Title of the parent unit." }
                                    }
                                }
                            }
                        }
            ...
            const theoryData = JSON.parse(theoryResponse.text || '{"units": []}');
            ...
            theoryData.units.forEach((u: any) => {
                ...
                const node: any = {
                    ...
                    title: u.title,
                    gist: u.gist,
                    category_id: u.type === 'ARC' ? NexusCategory.EVENT : (u.type === 'THEME' ? NexusCategory.CONCEPT : NexusCategory.META),
                ...
                if (isC) {
                    node.children_ids = [];
                    node.containment_type = u.type === 'ARC' ? ContainmentType.PLOT_ARC : ContainmentType.FOLDER;
        ...
        <div className="flex flex-col h-full bg-nexus-950 font-sans text-nexus-text">
            <header className="h-20 border-b border-nexus-800 bg-nexus-900/50 backdrop-blur-md flex items-center px-10 justify-between shrink-0 z-30 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-display font-black text-nexus-text tracking-tighter uppercase">Ekrixi <span className="text-nexus-accent">AI Blueprint</span></h1>
                        <p className="text-[10px] text-nexus-muted font-mono uppercase tracking-[0.3em]">Theoretical Drafting Core v5.0</p>
                    </div>
                </div>
                    ...
                    {stage === 'IDLE' && mode === 'PLANNING' && (
                        <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-8 duration-1000">
                            <div className="mb-12 space-y-4 text-center">
                                <h2 className="text-4xl font-display font-black text-nexus-text tracking-tight uppercase">The Dramatist's Ledger</h2>
                                <p className="text-nexus-muted text-lg font-serif italic leading-relaxed max-w-3xl mx-auto">
                                    "Seed a single premise. Select a narrative frame. Ekrixi AI will manifest a hierarchical sequence of arcs, chapters, and thematic anchors."
                                </p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                                {TEMPLATES.map(t => (
                                    <button 
                                        ...
                                        `}
                                    >
                                        <div className={`w-10 h-10 rounded-xl mb-4 flex items-center justify-center border transition-all ${selectedTemplate === t.id ? 'bg-nexus-accent text-white' : 'bg-nexus-950 border-nexus-800 text-nexus-muted group-hover:text-nexus-text'}`}>
                                            <t.icon size={20} />
                                        </div>
                                        <h3 className={`text-xs font-display font-black uppercase tracking-widest mb-2 transition-colors ${selectedTemplate === t.id ? 'text-nexus-accent' : 'text-nexus-text'}`}>{t.title}</h3>
                                        <p className="text-[10px] text-nexus-muted leading-relaxed font-serif italic">{t.desc}</p>
                                    </button>
                                ))}
                            </div>
                    ...
                    {stage === 'ANALYZING' && (
                        <div className="h-full flex flex-col items-center justify-center animate-in fade-in duration-700">
                            ...
                            <div className="max-w-md w-full space-y-8 text-center">
                                <h2 className="text-3xl font-display font-black text-nexus-text tracking-widest uppercase animate-pulse">{status}</h2>
                    ...
                    {stage === 'RESULT' && (
                        <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-6 duration-700">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                                <StatBox label="Narrative Arcs" val={blueprint.filter(i => (i as any).tags?.includes('ARC')).length} />
                                <StatBox label="Chapters Planned" val={blueprint.filter(i => (i as any).tags?.includes('CHAPTER')).length} />
                                <StatBox label="Thematic Pillars" val={blueprint.filter(i => (i as any).tags?.includes('THEME')).length} />
                            </div>
                            ...
                            <div className="flex-1 bg-nexus-900 border border-nexus-800 rounded-[40px] p-10 overflow-y-auto no-scrollbar shadow-2xl">
                                ...
                                <div className="space-y-12">
                                    {blueprint.filter(i => !isLink(i) && (i as any).tags?.includes('ARC')).map(arc => (
                                        <div key={arc.id} className="space-y-8">
                                            <div className="flex items-center gap-6 group">
                                                ...
                                                <div>
                                                    <h4 className="text-2xl font-display font-black text-nexus-text uppercase tracking-tight group-hover:text-nexus-accent transition-colors">{(arc as any).title}</h4>
                                                    <p className="text-[10px] text-nexus-muted font-mono uppercase tracking-[0.2em] mt-1">THEORY_BEAT: {(arc as any).tags?.[1]}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="ml-0 lg:ml-20 grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {blueprint.filter(i => 
                                                    ...
                                                    blueprint.some(l => isLink(l) && l.source_id === arc.id && l.target_id === i.id)
                                                ).map(ch => (
                                                    <div key={ch.id} className="bg-nexus-950/50 border border-nexus-800 rounded-3xl p-6 hover:border-nexus-accent/40 hover:bg-nexus-950 transition-all group/ch shadow-sm">
                                                        <div className="flex items-center gap-3 mb-4">
                                                            <BookOpen size={16} className="text-nexus-muted group-hover/ch:text-nexus-accent" />
                                                            <h5 className="text-sm font-display font-bold text-nexus-text uppercase">{(ch as any).title}</h5>
                                                        </div>
                                                        <p className="text-[11px] text-nexus-muted italic font-serif leading-relaxed line-clamp-2">"{(ch as any).gist}"</p>
                                                        
                                                        <div className="mt-4 pt-4 border-t border-nexus-800 flex flex-wrap gap-2">
                                                            {blueprint.filter(i => 
                                                                !isLink(i) && 
                                                                (i as any).tags?.includes('SCENE') && 
                                                                blueprint.some(l => isLink(l) && l.source_id === ch.id && l.target_id === i.id)
                                                            ).map(sc => (
                                                                <span key={sc.id} className="px-2 py-1 bg-nexus-900 border border-nexus-800 rounded text-[8px] font-mono text-nexus-muted uppercase tracking-widest font-bold">{(sc as any).title}</span>
                                                            ))}
                                                        </div>
                                                    </div>
                                                ))}
                                ...
                                <div className="mt-20 pt-10 border-t border-nexus-800/50">
                                    <h3 className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-[0.5em] mb-8">Thematic Foundations</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        {blueprint.filter(i => !isLink(i) && (i as any).tags?.includes('THEME')).map(theme => (
                                            <div key={theme.id} className="p-8 bg-nexus-accent/5 border border-nexus-accent/10 rounded-[32px] flex items-start gap-4 shadow-sm hover:border-nexus-accent/30 transition-all">
                                                <Flame size={20} className="text-nexus-accent shrink-0" />
                                                <div>
                                                    <h4 className="text-sm font-display font-black text-nexus-text uppercase tracking-wider">{(theme as any).title}</h4>
                                                    <p className="text-[10px] text-nexus-muted font-serif italic mt-2">"{(theme as any).gist}"</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
...
const StatBox = ({ label, val }: any) => (
    <div className="bg-nexus-900 border border-nexus-800 rounded-[32px] p-6 hover:border-nexus-accent/30 transition-all group shadow-sm">
        <div className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-widest mb-1 group-hover:text-nexus-accent transition-colors">{label}</div>
