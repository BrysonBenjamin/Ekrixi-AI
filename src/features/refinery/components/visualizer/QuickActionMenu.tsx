      ...
      <div className="relative w-full max-w-[320px] bg-nexus-900 border border-nexus-800 rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-5 pt-6 pb-4 text-center border-b border-nexus-800/50">
          <div className="flex justify-center mb-4">
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-lg border ${reified ? 'bg-orange-500/10 border-orange-500/20 shadow-orange-500/10' : 'bg-nexus-500/10 border-nexus-500/20 shadow-nexus-500/10'}`}
            >
              {getHeaderIcon()}
            </div>
          </div>
          ...
          {isL && !reified ? (
            ...
            <>
              <h3 className="text-lg font-bold text-white tracking-tight mb-0.5 truncate px-4">
                {title}
              </h3>
              <div
                className={`text-[9px] font-mono uppercase tracking-[0.2em] ${reified ? 'text-orange-500' : 'text-slate-500'}`}
              >
                {reified ? 'Reified Unit' : type}
              </div>
            </>
          )}
        </div>
...
const ActionButton = ({ icon: Icon, label, desc, onClick, color = 'text-nexus-500' }: any) => (
  ...
  >
    <div
      className={`p-2 rounded-lg bg-nexus-900 border border-nexus-800 group-hover:border-slate-700 transition-colors ${color}`}
    >
      <Icon size={16} />
    </div>
    <div className="text-left flex-1 min-w-0">
      ...
      <div className="text-[8px] text-slate-600 font-medium uppercase tracking-wider truncate">
        {desc}
      </div>
    </div>
    <ArrowRight
      size={12}
      className="text-slate-800 group-hover:text-slate-400 transition-all shrink-0"
    />
  </button>
);
