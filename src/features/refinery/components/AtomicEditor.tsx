...
export const AtomicEditor: React.FC<AtomicEditorProps> = ({
  ...
  const handleProtocolSwitch = (newType: NexusType) => {
    ...
    if (
      (newType === NexusType.HIERARCHICAL_LINK ||
    ...
    if (
      (newType === NexusType.AGGREGATED_SEMANTIC_LINK ||
        newType === NexusType.AGGREGATED_HIERARCHICAL_LINK) &&
      !reified
    ) {
    ...
    <div className="w-full max-w-4xl bg-nexus-900 border border-nexus-800 rounded-[40px] p-8 md:p-12 shadow-2xl animate-in fade-in slide-in-from-bottom-6 relative h-full overflow-y-auto no-scrollbar scroll-smooth">
      <div className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-5">
          <div
            className={`p-4 rounded-2xl border shadow-sm ${isL ? 'bg-nexus-arcane/10 border-nexus-arcane/20 text-nexus-arcane' : 'bg-nexus-accent/10 border-nexus-accent/20 text-nexus-accent'}`}
          >
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {reified && (
                <span className="px-2 py-0.5 rounded-full bg-nexus-accent/10 border border-nexus-accent/30 text-[8px] font-black text-nexus-accent uppercase tracking-widest">
                  Reified Manifest
                </span>
              )}
            </div>
        ...
        <div className="space-y-8 mb-12">
          <div className="flex items-center justify-between gap-8 p-10 bg-nexus-950 border border-nexus-800 rounded-[32px] shadow-inner relative overflow-hidden group">
            ...
            <div className="flex-1 text-center relative z-10">
              <div className="text-[9px] font-display font-black text-nexus-muted uppercase tracking-widest mb-3 opacity-60">
                Origin
              </div>
            ...
            <div className="flex flex-col items-center shrink-0 gap-3 relative z-10">
              <div
                className={`p-4 rounded-full border shadow-lg transition-all ${isHierarchical ? 'bg-nexus-essence/10 border-nexus-essence/30 text-nexus-essence' : 'bg-nexus-accent/10 border-nexus-accent/30 text-nexus-accent'}`}
              >
              ...
              <div className="space-y-4 animate-in slide-in-from-left-2 duration-300">
                ...
                >
                  <option value={HierarchyType.PARENT_OF}>
                    Parent Of (Origin contains Terminal)
                  </option>
      ...
      <div className="space-y-10">
        <div className="space-y-4">
          <label className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-[0.2em] ml-2 opacity-60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <label className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-[0.2em] ml-2 flex items-center gap-2 opacity-60">
        ...
        <div className="space-y-4 relative">
          <label className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-[0.2em] ml-2 flex items-center justify-between opacity-60">
            ...
            <span className="text-[8px] italic font-mono flex items-center gap-1.5">
              <ShieldCheck size={10} className="text-nexus-essence" /> Encrypted Storage
            </span>
          </label>
            ...
            {atMenu && suggestions.length > 0 && (
              <div className="absolute bottom-full left-0 mb-4 w-[320px] bg-nexus-900 border border-nexus-700 rounded-[32px] shadow-2xl overflow-hidden backdrop-blur-3xl animate-in fade-in zoom-in-95 duration-200 z-[100]">
                <div className="px-6 py-4 border-b border-nexus-800 flex items-center justify-between bg-nexus-950/40">
                  <span className="text-[10px] font-display font-black text-nexus-accent uppercase tracking-[0.2em] flex items-center gap-2">
                    <Sparkles size={14} /> Registry Search
                  </span>
                  <button
