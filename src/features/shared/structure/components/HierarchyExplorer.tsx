...
export const HierarchyExplorer: React.FC<HierarchyExplorerProps> = ({
  ...
  const { roots, registry, reifiedLinksByOrigin } = useMemo(() => {
    ...
    items.forEach((item) => {
      reg[item.id] = item;
      if (isReified(item) && isBinaryLink(item)) {
    ...
    const rootNodes = items.filter((item) => {
      ...
      if (isReified(item)) {
        const hasSourceParent =
          isBinaryLink(item) && (item as any).source_id && reg[(item as any).source_id];
  ...
  const renderNode = (
    ...
    const isAuthorNote = node._type === NexusType.AUTHOR_NOTE;
    ...
    const isTimeNode = !!(node as SimpleNote).time_state?.is_historical_snapshot;
          ...
          {isFolder && isOpen && (
            <div className="flex flex-col mt-1">
              {node.children_ids.map((cid) => {
                ...
                if (child && (child as SimpleNote).time_state?.is_historical_snapshot) return null;
