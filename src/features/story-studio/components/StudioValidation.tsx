...
export const StudioValidation: React.FC<StudioValidationProps> = ({
  delta,
  studioItems,
  worldRegistry,
}) => {
    ...
    <div className="h-full bg-nexus-950 p-12 overflow-y-auto no-scrollbar animate-in fade-in duration-1000">
      <div className="max-w-5xl mx-auto space-y-12">
        <header className="text-center space-y-4 mb-20">
          ...
          <h2 className="text-5xl font-display font-black text-nexus-text tracking-tighter uppercase leading-none">
            The Soul <span className="text-nexus-ruby">Audit.</span>
          </h2>
          <p className="text-nexus-muted text-lg font-serif italic max-w-2xl mx-auto opacity-70 leading-relaxed">
            "Measuring the distance between Alpha and Omega. Validating thematic consistency across
            reified logic bridges."
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-nexus-900 border border-nexus-800 p-8 rounded-[40px] shadow-xl space-y-6 relative overflow-hidden group">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-display font-black text-nexus-muted uppercase tracking-widest">
                Narrative Mass Delta
              </span>
            </div>
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                <div className="text-4xl font-display font-black text-nexus-text tracking-tighter">
                  {delta?.startWeight} → {delta?.endWeight}
                </div>
                <div className="text-[10px] font-mono text-nexus-essence font-bold">
                  ESTIMATED GAIN: +400%
                </div>
              </div>
              <div className="w-full h-1.5 bg-nexus-950 rounded-full overflow-hidden border border-nexus-800">
                <div
                  className="h-full bg-nexus-ruby shadow-[0_0_10px_rgba(225,29,72,0.5)]"
          ...
          <div className="bg-nexus-900 border border-nexus-800 p-8 rounded-[40px] shadow-xl space-y-6">
            ...
            <div className="text-4xl font-display font-black text-nexus-text tracking-tighter">
              {mentions.length} Nodes
            </div>
            <p className="text-xs text-nexus-muted font-medium leading-relaxed">
              {mentions.length > 5
                ? 'High causal density. World nodes are properly influencing the narrative arc.'
                : 'Low density. Consider referencing more world signatures to anchor the lore.'}
            </p>
          </div>
...
const InsightItem = ({ type, icon: Icon, title, content }: InsightItemProps) => (
  ...
  >
    <div
      className={`p-3 rounded-2xl ${type === 'WARNING' ? 'bg-red-500/10 text-red-500' : 'bg-nexus-essence/10 text-nexus-essence'}`}
    >
      <Icon size={20} />
    </div>
    <div>
      <h4 className="text-sm font-display font-bold text-nexus-text uppercase tracking-tight mb-1">
        {title}
      </h4>
