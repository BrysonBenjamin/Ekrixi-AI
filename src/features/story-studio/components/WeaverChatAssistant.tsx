  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemInstruction = `
                ...
                USE "[[Title]]" SYNTAX in your conversational replies to reference world elements.
                
                REQUIREMENT: Return RAW PROSE only in the 'proseSuggestion' field. Do not use markdown there.
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          reply: 'Assistant synchronization interrupted. Registry heartbeat lost.',
        },
      ]);
    } finally {
      setIsLoading(false);
        ...
        {messages.length === 0 && (
          <div className="py-20 text-center space-y-6 opacity-30">
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-display font-bold uppercase tracking-[0.2em] px-10">
                Awaiting Writing Directives
              </p>
              <p className="text-[9px] font-serif italic max-w-xs mx-auto">
                "Highlight text in the editor and click 'Search Selection' to send it as context."
              </p>
            </div>
          </div>
        )}
          ...
          >
            {m.selection && (
              <div className="max-w-[80%] mb-2 p-3 bg-nexus-accent/10 border border-nexus-accent/30 rounded-2xl flex items-start gap-3">
                ...
                <p className="text-[10px] text-nexus-text italic line-clamp-3 leading-relaxed">
                  "{m.selection}"
                </p>
              </div>
            )}
            ...
            <div
              className={`max-w-[90%] p-5 rounded-[28px] shadow-sm text-xs leading-relaxed ${m.role === 'user' ? 'bg-nexus-accent text-white font-bold' : 'bg-nexus-950 border border-nexus-800 text-nexus-text font-serif italic'}`}
            >
              {m.role === 'user' ? (
                m.text
              ) : (
              ...
              <div className="mt-4 w-full bg-nexus-950/80 border border-nexus-accent/30 rounded-[32px] p-6 shadow-xl space-y-4 group relative overflow-hidden">
                ...
                {m.logicObservation && (
                  <div className="pt-4 border-t border-nexus-800 flex items-start gap-3">
                    ...
                    <p className="text-[9px] text-amber-500/80 font-mono uppercase tracking-tighter leading-tight">
                      {m.logicObservation}
                    </p>
                  </div>
                )}
      ...
      <div className="p-5 border-t border-nexus-800 bg-nexus-950/60 relative">
        {selection && (
          <div className="absolute bottom-full left-4 right-4 mb-4 p-4 bg-nexus-accent text-white rounded-[24px] shadow-2xl flex items-center justify-between animate-in slide-in-from-bottom-2 duration-300">
            ...
            <button
              ...
              className="p-1 hover:bg-white/20 rounded-full shrink-0 ml-2"
            >
              <X size={14} />
            </button>
          </div>
        ...
        <div className="relative group">
          <input
            ...
            placeholder={
              selection
                ? 'Suggest a rewrite for selection...'
                : 'Request a scene expansion or edit...'
            }
