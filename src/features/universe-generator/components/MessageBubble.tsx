...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  return (
    <div className={`flex w-full mb-10 group ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`flex flex-col max-w-[95%] md:max-w-[85%] ${isUser ? 'items-end' : 'items-start'}`}
      >
        {/* 1. Header Metadata */}
        {!isEditing && (
          <div
            className={`flex items-center gap-2 mb-2 opacity-60 px-2 ${isUser ? 'flex-row-reverse' : ''}`}
          >
            {isUser ? (
              <User size={12} className="text-nexus-accent" />
            ) : (
            ...
            <div className="w-full bg-nexus-900 border border-nexus-800 rounded-2xl p-5 shadow-2xl min-w-[280px] md:min-w-[500px]">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 mt-4 border-t border-nexus-800">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl text-[10px] font-display font-black bg-nexus-accent text-white hover:bg-nexus-text transition-all flex items-center gap-2 uppercase tracking-widest shadow-lg shadow-nexus-accent/10"
                >
